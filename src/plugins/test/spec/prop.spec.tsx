...
export function testCallbagPropSupport<N, R extends LiveRendererLike<N>>(
  factory: (...plugins: PluginFactory<N, RendererLike<N>>[]) => R,
  getProp: (n: N, key: string) => string,
  livelyRoot: () => N,
) {
  it('should update contents using given callbag.', () => {
    ...
    const div = <div title={c}/>;
    renderer.render(div).on(livelyRoot());
    expect(getProp(div, 'title')).to.equal('Hellow World!');
    c.set('Goodbye Blue Sky!');
    expect(getProp(div, 'title')).to.equal('Goodbye Blue Sky!');
  });
  ...
  it('should unsubscribe from given callbag when the node is removed.', done => {
    ...
    const div = <div title={c}/>;
    renderer.render(div).on(livelyRoot());
    getProp(div, 'title').should.equal('All work and no play');
    renderer.remove(div);
  });
  ...
  it('should properly handle already terminated callbags on removal.', () => {
    ...
    const div = <div title={c}/>;
    renderer.render(div).on(livelyRoot());
    getProp(div, 'title').should.equal('Halo meine liebe');
    c(2);
    renderer.remove(div);
  });
  ...
  it('should only update when value of the callbag is changed (by reference).', () => {
    ...
    const div = <div title={c}/>;
    renderer.render(div).on(livelyRoot());
    expect(getProp(div, 'title')).to.equal('1,2,3');
    c.sub(1).set(4);
    expect(getProp(div, 'title')).to.equal('1,2,3');
